...
def JCMF_Double(res_dir, train_user, train_item, valid_user, test_user,
    ...
    print "item_bais:",   item_bais[0:10]
    print "######################################"
   ...
   iidex=0
    ...
    for iteration in xrange(max_iter):
        ...
        for i in xrange(num_user):
            ...
            Y_i = YY[idx_item]
            R_i = Train_R_I[i]
            A = t_YY + (a - b) * (Y_i.T.dot(Y_i))
            B = (a * Y_i * (np.tile(R_i, (dimension, 1)).T)).sum(0)
            B = B - P[i].dot(Y_i.T.dot(Y_i))
            ...
            # approx_R_i = XX[i].dot(Y_i.T)
            # U[i]=U[i]+eta*(((V_i * (np.tile(-R_i+approx_R_i, (dimension, 1)).T)).sum(0) )+lambda_u*U[i])
            SA =  (a - b) * (Q_i.T.dot(Q_i))+lambda_p * np.eye(dimension)+Y_i.T.dot(Y_i)
            s1= (a * Y_i * (np.tile(S_R_i, (dimension, 1)).T)).sum(0)
            s2=-U[i].dot(Y_i.T.dot(Y_i))
            s3=(Q_i * (np.tile(R_i, (dimension, 1)).T)).sum(0)
            SB =s1+s2+s3 
        ...
        for j in xrange(num_item):
            ...
            tmp_A =  (a - b) * (XX_j.T.dot(XX_j))+ lambda_v * np.eye(dimension)
            A = tmp_A + lambda_v * item_weight[j] * np.eye(dimension)
            B = (a * XX_j * (np.tile(R_j, (dimension, 1)).T)
                 ).sum(0) + lambda_v * item_weight[j] * theta[j]
            B = B -Q[j].dot(XX_j.T.dot(XX_j))
            ...
            SA =  (a - b) * (P_j.T.dot(P_j))+ lambda_q* np.eye(dimension)+XX_j.T.dot(XX_j)
            SB = (a * XX_j * (np.tile(R_j, (dimension, 1)).T)).sum(0)
            SB = SB +( P_j * (np.tile(S_R_j, (dimension, 1)).T)).sum(0)
            SB = SB - V[j].dot(XX_j.T.dot(XX_j))
        ...
        if te_mae < better_mae:
            better_mae = te_mae
        print "\ JCMF_Double=============better_rmse:{}=====better_mae:{}==============\n".format(better_rmse,better_mae)
