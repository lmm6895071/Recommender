...
def JCONMF_P(res_dir, train_user, train_item, valid_user, test_user,
           R, CNN_X, vocab_size, init_W=None, give_item_weight=True,
           max_iter=50, lambda_u=1, lambda_v=100, dimension=50,
           dropout_rate=0.2, emb_dim=200, max_len=300, num_kernel_per_ws=100,lambda_p=10,lambda_q=1):
    # explicit setting
    a = 1
    b = 0
    eta=-0.0005
    ...
    stddv=0.8
    Q = np.random.uniform(0.001,stddv, size=(num_item,dimension))
    P = np.random.uniform(0.001,stddv,size=(num_user,dimension))
    M = np.random.uniform(0.001,stddv, size=(num_user,dimension))
    Gama=np.random.uniform(0.001,stddv, size=(dimension,dimension))
    ...
    for iteration in xrange(max_iter):
        ...
        loss = loss + np.sum(sub_loss)-np.sum(np.square(M)) 
        print "=================================================================="
        print "the shape of V, V[i] {} {}".format(V.shape,V[0].shape)
        ...
        for j in xrange(num_item):
            ...
            '''
            try:
                ming_test=np.linalg.cholesky(SA)
            except Exception as err:
                print err
                print "UU+lambda_v*E  is not a positive matix"
            '''
